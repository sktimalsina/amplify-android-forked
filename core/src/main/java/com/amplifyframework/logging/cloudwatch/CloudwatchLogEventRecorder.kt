...
import com.amplifyframework.logging.cloudwatch.persistence.CloudWatchLoggingDatabase
    ...
    suspend fun saveLogEvent(event: CloudWatchLogEvent) {
        if (!stopSync) {
            try {
                ...
                Log.e("CloudwatchLogEventRecorder", "failed to save event, error $e, stacktrace:  ${Log.getStackTraceString(e)}")
            }
        ...
        withContext(dispatcher) {
            awsCloudWatchLogsClient?.let { client ->
                ...
                if (inputLogEvents.size < 1)
                    return@withContext
                val response = client.describeLogStreams(DescribeLogStreamsRequest {
